...
class EmployeeCRUDCBV(HttpResponseMixin, SerializeMixin, View):
    ...
    def put(self, request, *args, **kwargs):
        ...
        id = data.get('id', None)
        if id is None:
            return self.render_to_http_response(json.dumps({'msg': 'To perform updating id is mandatory, you should provide'}), status=400)
        obj = self.get_object_by_id(id)
        if obj is None:
            json_data = json.dumps({'msg': 'No matched record found, Not possible to perform update'})
